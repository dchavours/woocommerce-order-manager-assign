...
$sql_parent_array = "SELECT post_parent, post_date,post_status, post_name, post_type FROM {$wpdb->prefix}posts WHERE ID IN ('.$ids.')";
...
SELECT meta_key, meta_value, post_id  FROM {$wpdb->prefix}postmeta WHERE post_id IN ('.$ids.')
AND meta_key NOT IN
